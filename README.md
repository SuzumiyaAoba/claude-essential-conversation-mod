...
- A prompt is split on `<pasted_content id="...">...</pasted_content id="...">` spans (what the composer wraps a paste in — the closing tag repeats the id, not the bare `</pasted_content>` an XML reader would expect), depth-counted rather than matched by a lazy regex so a paste that itself quotes `<pasted_content>` text doesn't end the span early. Each span starts collapsed to a one-line "N chars — click to expand" placeholder with its own fold toggle; expanded, its (trimmed) text draws inside its own bordered box, instead of dumping the whole paste into the card as bare text.
